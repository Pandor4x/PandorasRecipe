...
// API base for all frontend requests
const API_URL = "https://pandorasrecipe.onrender.com";

// Helper: resolve image URL to an absolute URL when needed
function resolveImageUrl(url) {
  if (!url) return '';
  // data URLs and absolute HTTP(s) URLs should be returned as-is
  if (typeof url !== 'string') return '';
  const trimmed = url.trim();
  if (trimmed.startsWith('data:') || trimmed.startsWith('http://') || trimmed.startsWith('https://')) return trimmed;
  // If it's a server-relative uploads path like `/uploads/...`, prefix API_URL
  if (trimmed.startsWith('/')) return API_URL.replace(/\/$/, '') + trimmed;
  // otherwise return as-is (may be already relative to the frontend)
  return trimmed;
}
...
async function renderHomeRecipes(recipes = null) {
  ...
  async function renderHomeCategory(category) {
    ...
    try {
      ...
      recipes.forEach(r => {
        // normalize image URL for this recipe so templates can use it
        if (r && r.image) r._image = resolveImageUrl(r.image);
        ...
        card.innerHTML = `
          <div class="recipe-card-flex">
            ${r.image ? `<img src="${r._image || ''}" alt="Recipe Image" class="recipe-img-side">` : `<div class="recipe-img-side placeholder"></div>`}
...
async function buildHomeSlideshow() {
  try {
    const recipes = await fetchRecipes();
    const imgs = recipes.filter(r => r.image).map(r => ({url: resolveImageUrl(r.image), title: r.title || '', id: r.id}));
  ...
  try {
    ...
    for (const [id, list] of mapping) {
      ...
      list.forEach(r => {
        ...
        front.innerHTML = `
          ${r.image ? `<img src="${r._image || ''}" style="width:100%; height:140px; object-fit:cover; border-radius:10px;">` : `<div style="height:140px;background:#eee;border-radius:10px"></div>`}
    ...
    img.onload = async () => {
      ...
      try {
        ...
        if (!uploadRes.ok) {
          ...
          // Prefer server-returned absolute URL. If server provides only filename, build absolute URL.
          if (uploadData && uploadData.url) {
            imageDataUrl = uploadData.url;
          } else if (uploadData && uploadData.filename) {
            imageDataUrl = `${API_URL.replace(/\/$/, '')}/uploads/${uploadData.filename}`;
          } else {
            imageDataUrl = dataUrl;
          }
        }
        previewImage.src = resolveImageUrl(imageDataUrl);
...
window.editRecipe = async function(id) {
  ...
  previewImage.src = resolveImageUrl(r.image) || '';
